...
data modify storage minecraft:animation TempBlock.Temp.Name set from entity @e[tag=rc_item_holder,limit=1] HandItems[0].id
data modify storage minecraft:animation TempBlock.Temp.Properties set from entity @e[tag=rc_item_holder,limit=1] HandItems[0].tag.BlockStateTag
data modify storage minecraft:animation TempBlock.Temp.TileEntityData set from entity @e[tag=rc_item_holder,limit=1] HandItems[0].tag.BlockEntityTag
kill @e[tag=rc_item_holder]

#Different item from the block
execute if block ~ ~ ~ #rc:corals[waterlogged=false] run data modify storage minecraft:animation TempBlock.Temp.Properties set value {waterlogged:"false"}

execute if block ~ ~ ~ fire run data modify storage minecraft:animation TempBlock.Temp.Name set value "minecraft:fire"
execute if block ~ ~ ~ water run data modify storage minecraft:animation TempBlock.Temp.Name set value "minecraft:water"
execute if block ~ ~ ~ lava run data modify storage minecraft:animation TempBlock.Temp.Name set value "minecraft:lava"
execute if block ~ ~ ~ redstone_wire run data modify storage minecraft:animation TempBlock.Temp.Name set value "minecraft:redstone_wire"
execute if block ~ ~ ~ tripwire run data modify storage minecraft:animation TempBlock.Temp.Name set value "minecraft:tripwire"
execute if block ~ ~ ~ powder_snow run data modify storage minecraft:animation TempBlock.Temp.Name set value "minecraft:powder_snow"
